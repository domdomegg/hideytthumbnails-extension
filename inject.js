const css = {
  ...
  "blurred": "ytd-thumbnail img, ytd-playlist-thumbnail img { filter: blur(16px); }",
}
...
const updateElem = async () => {
  ...
  const isDisabled = options.disabledOnPages.everywhere
    || (options.disabledOnPages.search && window.location.pathname === '/results')
  ...
  elem.innerHTML = `/* Injected by hideytthumbnails.js */
